...
public class EvaluateTest {
        ...
        for(var model:models) {
            for (int i = MIN_PROCESS; i <= MAX_PROCESS; i++) {
                for (var zoneRep : List.of(ClockStrategy2.ZoneRepresentation.Global, ClockStrategy2.ZoneRepresentation.Local, ClockStrategy2.ZoneRepresentation.LocalSyncSub)) {
                    result.add(new Object[]{model, zoneRep, false, i});
                }
                result.add(new Object[]{model, ClockStrategy2.ZoneRepresentation.Local, true, i});
                result.add(new Object[]{model, ClockStrategy2.ZoneRepresentation.LocalSyncSub, true, i});
            }
        }
        return result;
    }
    ...
    public void before() throws Exception {
        for (var t : List.of("Global-NOPOR", "Local-NOPOR", "Local-POR", "LocalSyncSub-NOPOR", "LocalSyncSub-POR")) {
            FileWriter fw = new FileWriter(String.format(OUTPUT_TEMPLATE, model.modelName, t), false);
    ...
    public void evaluate() throws IOException {
        ...
        FileWriter fw = new FileWriter(String.format(OUTPUT_TEMPLATE, model.modelName, String.format("%s-%s", zoneRepresentation.toString(), isPor ? "POR" : "NOPOR")), true);
        BufferedWriter bw = new BufferedWriter(fw);
