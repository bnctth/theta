...
public abstract class XtaActionWithId extends StmtAction {
    ...
    private XtaActionWithId(final XtaSystem system, final List<Loc> source) {
        checkNotNull(system);
    ...
    public static BasicXtaAction basic(final XtaSystem system, final List<Loc> sourceLocs, final Edge edge, final int sourceProcessId, ProcessIdProvider processIdProvider) {
        return new BasicXtaAction(system, sourceLocs, edge, sourceProcessId, processIdProvider);
    }

    public static BinaryXtaAction binary(final XtaSystem system, final List<Loc> sourceLocs, final Edge emitEdge,
                                         final Edge recvEdge, final int sourceProcessId, ProcessIdProvider processIdProvider) {
        return new BinaryXtaAction(system, sourceLocs, emitEdge, recvEdge, sourceProcessId, processIdProvider);
    }

    public static BroadcastXtaAction broadcast(final XtaSystem system, final List<Loc> sourceLocs, final Edge emitEdge,
                                               final List<Edge> recvEdges, final int sourceProcessId, ProcessIdProvider processIdProvider) {
        return new BroadcastXtaAction(system, sourceLocs, emitEdge, recvEdges, sourceProcessId, processIdProvider);
    }
    ...
    public static final class BasicXtaAction extends XtaActionWithId {
        private final Edge edge;
        private final List<Loc> targetLocs;

        private volatile List<Stmt> stmts = null;

        private BasicXtaAction(final XtaSystem system, final List<Loc> sourceLocs, final Edge edge, final int sourceProcessId, ProcessIdProvider processIdProvider) {
            super(system, sourceLocs);
            this.edge = checkNotNull(edge);

            final Builder<Loc> builder = ImmutableList.builder();
            final Loc source = edge.getSource();
            final Loc target = edge.getTarget();
            boolean matched = false;
            for (int i = 0; i < sourceLocs.size(); i++) {
                final Loc loc = sourceLocs.get(i);
                if (loc.equals(source)) {
                    checkArgument(!matched);
                    builder.add(target);
                    matched = true;
                    processId = processIdProvider.newIdForBasicAction(i, sourceProcessId);
                } else {
                    builder.add(loc);
                }
            }
        ...
        public List<Stmt> getStmts() {
            ...
            if (stmts == null) {
                final Builder<Stmt> builder = ImmutableList.builder();
        ...
        public int hashCode() {
            ...
            if (result == 0) {
                ...
                result = 31 * result + ((Integer) getProcessId()).hashCode();
                hashCode = result;
            }
            return result;
        }
        ...
        public boolean equals(final Object obj) {
            if (this == obj) {
                ...
                return this.getClockVars().equals(that.getClockVars())
                        ...
                        && this.getProcessId() == that.getProcessId();
            } else {
                return false;
            }
        }
        ...
        public String toString() {
            return Utils.lispStringBuilder(getClass().getSimpleName()).body().addAll(edge.getSelections())
                    .addAll(edge.getGuards()).addAll(edge.getUpdates()).add(getProcessId()).toString();
        }

    }

    public static final class BinaryXtaAction extends XtaActionWithId {
        ...
        private BinaryXtaAction(final XtaSystem system, final List<Loc> sourceLocs, final Edge emitEdge,
            ...
            int recvProcess = -1;
            for (int i = 0; i < sourceLocs.size(); i++) {
                Loc loc = sourceLocs.get(i);
                if (loc.equals(emitSource)) {
                    checkArgument(emitProcess == -1);
                    builder.add(emitTarget);
                    emitProcess = i;
                } else if (loc.equals(recvSource)) {
                    checkArgument(recvProcess == -1);
            ...
            checkArgument(emitProcess != -1);
            checkArgument(recvProcess != -1);
            processId = processIdProvider.newIdForBinaryAction(emitProcess, recvProcess, sourceProcessId);
            targetLocs = builder.build();
        }
        ...
        public String toString() {
            return Utils.lispStringBuilder(getClass().getSimpleName())
                    ...
                    .addAll(emitEdge.getUpdates()).addAll(recvEdge.getUpdates()).add(getProcessId()).toString();
        }

    }

    public static final class BroadcastXtaAction extends XtaActionWithId {
        private final Edge emitEdge;
        ...
        private BroadcastXtaAction(final XtaSystem system, final List<Loc> sourceLocs, final Edge emitEdge, List<Edge> recvEdges, final int sourceProcessId, ProcessIdProvider processIdProvider) {
            super(system, sourceLocs);
            this.emitEdge = checkNotNull(emitEdge);
            ...
            final Builder<Collection<Edge>> nonRecvEdgesBuilder = ImmutableList.builder();
            final Builder<Loc> targetLocsBuilder = ImmutableList.builder();
            final Loc emitSource = emitEdge.getSource();
            final Loc emitTarget = emitEdge.getTarget();


            int emitProcess = -1;
            final Builder<Integer> recvProcesses = ImmutableList.builder();
            ...
            for (int i = 0; i < sourceLocs.size(); i++) {
                ...
                if (loc.equals(emitSource)) {
                    ...
                    emitProcess = i;
                } else if (optRecvEdge.isEmpty() || !optRecvEdge.get().getSource().equals(loc)) {
            ...
            assert emitProcess != -1;
            assert recvsMatched;

            nonRecvEdges = nonRecvEdgesBuilder.build();
            targetLocs = targetLocsBuilder.build();
            processId = processIdProvider.newIdForBroadcastAction(emitProcess, recvProcesses.build(), sourceLocs.size(), sourceProcessId);
    ...
    private static void addClocksNonNegative(final Builder<Stmt> builder,
                                             final Collection<VarDecl<RatType>> clocks) {
        clocks.forEach(c -> builder.add(Assume(Geq(c.getRef(), Rat(0, 1)))));
    }

    private static void addInvariants(final Builder<Stmt> builder, final List<Loc> locs) {
        locs.forEach(l -> l.getInvars().forEach(i -> builder.add(Assume(i.toExpr()))));
    }

    private static void addSelections(final Builder<Stmt> builder, final Edge edge) {
    ...
    private static void addGuards(final Builder<Stmt> builder, final Edge edge) {
        edge.getGuards().forEach(g -> builder.add(Assume(g.toExpr())));
    }


    private static void addNonRecvSyncAndGuards(final Builder<Stmt> builder, final Edge emitEdge, final Edge nonRecvEdge) {
        final Stream<Expr<?>> emitArgs = emitEdge.getSync().get().getArgs().stream();
    ...
    private static void addUpdates(final Builder<Stmt> builder, final Edge edge) {
        edge.getUpdates().forEach(u -> builder.add(u.toStmt()));
    }

    private static void addDelay(final Builder<Stmt> builder, final Collection<VarDecl<RatType>> clocks) {
